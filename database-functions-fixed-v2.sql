-- Database Functions for PNP Dashboard - Fixed Version 2
-- This version maintains all original functionality with proper type handling

-- Drop existing functions first to ensure clean creation
...
BEGIN
  ...
  WHERE c.assigned_officer_id = p_officer_id
    AND c.status = ANY(ARRAY['Pending', 'Under Investigation', 'Requires More Information'])
...
BEGIN
  ...
  WITH current_month AS (
    SELECT 
      ...
      AVG(EXTRACT(DAY FROM (updated_at - created_at))) FILTER (WHERE status = ANY(ARRAY['Resolved', 'Dismissed'])) AS avg_resolution_days
    FROM complaints
  ...
  SELECT 
    ...
    COALESCE(cm.resolved_this_month, 0) AS current_value,
    COALESCE(pm.resolved_last_month, 0) AS previous_value,
    CASE 
      WHEN COALESCE(cm.resolved_this_month, 0) > COALESCE(pm.resolved_last_month, 0) THEN 'up'
      WHEN COALESCE(cm.resolved_this_month, 0) < COALESCE(pm.resolved_last_month, 0) THEN 'down'
      ELSE 'stable'
  ...
  SELECT 
    ...
    COALESCE(cm.new_cases_this_month, 0) AS current_value,
    COALESCE(pm.new_cases_last_month, 0) AS previous_value,
    CASE 
      WHEN COALESCE(cm.new_cases_this_month, 0) > COALESCE(pm.new_cases_last_month, 0) THEN 'up'
      WHEN COALESCE(cm.new_cases_this_month, 0) < COALESCE(pm.new_cases_last_month, 0) THEN 'down'
      ELSE 'stable'
    END AS trend_direction,
    CASE 
  ...
  SELECT 
    ...
    ROUND(COALESCE(cm.avg_resolution_days, 0), 1) AS current_value,
    ROUND(COALESCE(pm.avg_resolution_days, 0), 1) AS previous_value,
    CASE 
      WHEN COALESCE(cm.avg_resolution_days, 0) < COALESCE(pm.avg_resolution_days, 0) THEN 'down' -- Lower is better
      WHEN COALESCE(cm.avg_resolution_days, 0) > COALESCE(pm.avg_resolution_days, 0) THEN 'up'
      ELSE 'stable'
    END AS trend_direction,
    CASE 
...
BEGIN
  ...
  FROM complaints c
  WHERE c.assigned_officer_id = p_officer_id
    AND c.status = ANY(ARRAY['Pending', 'Under Investigation'])
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM complaints WHERE assigned_officer_id = p_officer_id AND status = ANY(ARRAY['Pending', 'Under Investigation', 'Requires More Information']))::INTEGER AS total_active_cases;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Test if functions exist after creation
SELECT 
