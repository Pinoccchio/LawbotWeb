-- Fixed Database Functions for PNP Dashboard Real Data
...
BEGIN
  ...
  WHERE c.assigned_officer_id::TEXT = p_officer_id::TEXT
...
BEGIN
  ...
  WITH current_month AS (
    ...
    FROM complaints
    WHERE assigned_officer_id::TEXT = p_officer_id::TEXT
...
BEGIN
  ...
  SELECT 
    ...
    (SELECT COUNT(*) FROM complaints WHERE assigned_officer_id::TEXT = p_officer_id::TEXT AND DATE(created_at) = CURRENT_DATE)::INTEGER AS new_cases_today,
    (SELECT COUNT(*) FROM complaints WHERE assigned_officer_id::TEXT = p_officer_id::TEXT AND status = 'Requires More Information' AND last_citizen_update > updated_at)::INTEGER AS pending_updates,
    (SELECT COUNT(*) FROM complaints WHERE assigned_officer_id::TEXT = p_officer_id::TEXT AND status IN ('Pending', 'Under Investigation', 'Requires More Information'))::INTEGER AS total_active_cases;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
GRANT EXECUTE ON FUNCTION get_officer_weekly_activity TO anon;
GRANT EXECUTE ON FUNCTION get_officer_performance_metrics TO anon;
GRANT EXECUTE ON FUNCTION get_officer_upcoming_tasks TO anon;
GRANT EXECUTE ON FUNCTION get_officer_notification_summary TO anon;